...
/// Tralala!
#[async_recursion]
pub async fn get_vector(syn: SequenceSyntax) -> Box<dyn Sequence<f64, dyn Send> + Send> {
    let zaporedje: Box<dyn Sequence<f64, dyn Send> + Send> = match (syn).name.clone() {
        s if s == "/sequence/Sum".to_string() => {
            let a = get_vector(*(syn.sequences[0].clone())).await;
            let b = get_vector(*(syn.sequences[1].clone())).await;
